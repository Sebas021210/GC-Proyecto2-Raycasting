...
public:
    ...
    static void init() {
        int imgFlags = IMG_INIT_JPG; // or IMG_INIT_JPG, depending on your needs
    ...
    static void loadImage(const std::string& key, const char* path) {
        ...
        if (!newSurface) {
            throw std::runtime_error("Unable to load image! SDL_image Error: " + std::string(IMG_GetError()) + " Path: " + std::string(path));
        }
